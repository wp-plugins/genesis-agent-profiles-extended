...
class Custom_AgentEvolution_Profiles_Widget extends AgentEvolution_Profiles_Widget {
    ...
    function widget($args, $instance) {
        ...
        /** defaults */
        $instance = wp_parse_args($instance, array('post_id' => '', 'title' => '', 'show_agent' => 0, 'show_number' => 0, 'orderby' => '', 'order' => ''));
        ...
        if (empty($show_number)) $show_number = - 1;
        ...
        elseif ($show_agent == 'show_random') {
            ...
            echo $before_title . apply_filters('widget_title', $title, $instance, $this->id_base) . $after_title;
            $query_args = array('post_type' => 'aeprofiles', 'posts_per_page' => $show_number, 'orderby' => 'rand', 'order' => $order);
        } 
        elseif (!empty($instance['post_id'])) {
            $post_id = explode(',', $instance['post_id']);
            echo $before_title . apply_filters('widget_title', $title, $instance, $this->id_base) . $after_title;
            $query_args = array('post_type' => 'aeprofiles', 'p' => $post_id[0], 'posts_per_page' => 1, 'orderby' => $orderby, 'order' => $order);
        }
        ...
        if (have_posts()):
            while (have_posts()):
                ...
                if ($show_agent !== 'show_selected') echo '<div ', post_class('widget-agent-wrap'), '>';
                ...
                if ($show_agent !== 'show_selected') echo '</div><!-- .widget-agent-wrap -->';
            endwhile;
        endif;
        wp_reset_query();
...
?>
    ...
    <p><hr></p>
         ...
         <label for="<?php
        echo $this->get_field_id('show_number'); ?>"><?php
        _e('Max number of agents to show:'); ?></label>
        <input id="<?php
        echo $this->get_field_id('show_number'); ?>" name="<?php
        echo $this->get_field_name('show_number'); ?>" type="text" value="<?php
